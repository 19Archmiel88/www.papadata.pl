...
const IntegrationsModal: React.FC<Props> = ({ 
  ...
  // Effect for escape key, focus trap and scroll lock
  useEffect(() => {
    if (!isOpen) return;
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') {
        ...
        const focusable = modalRef.current?.querySelectorAll<HTMLElement>('a, button, textarea, input, select, [tabindex]:not([tabindex="-1"])');
        ...
        >
          ...
          <div className="p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 id="integrations-modal-title" className="text-2xl font-bold text-slate-900 dark:text-white">{t.title}</h2>
                <p className="text-slate-600 dark:text-slate-400 mt-2 text-sm max-w-2xl">{t.description}</p>
              </div>
              <button onClick={onClose} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-lg transition-colors" aria-label={lang === 'PL' ? 'Zamknij' : 'Close'}>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                  {categories.map(cat => (
                    <button
                      ...
                      `}
                      aria-label={`${isOpen && lang === 'PL' ? 'Filtruj integracje' : 'Filter integrations'}: ${getCategoryName(cat)}`}
                    >
                      {getCategoryName(cat)}
                    </button>
                  ))}
                    ...
                    >
                      ...
                      <div className="ml-4">
                        {item.status === 'Available' && (
                          <button 
                            ...
                            aria-label={isLoggedIn 
                              ? (lang === 'PL' ? `Połącz ${item.name}` : `Connect ${item.name}`) 
                              : (lang === 'PL' ? 'Zaloguj się, aby połączyć' : 'Log in to connect')}
                          >
                        ...
                        {item.status === 'ComingSoon' && (
                          <button disabled className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-400 text-sm font-medium rounded-lg cursor-not-allowed" aria-label="Coming Soon">
                        ...
                        {item.status === 'Voting' && (
                          <button 
                            ...
                            aria-label={lang === 'PL' ? 'Zagłosuj' : 'Vote'}
                          >
                        ...
                        <div className="w-full space-y-4 text-left">
                          ...
                          <div className="flex gap-3 pt-2">
                            <button
                              ...
                              aria-label={lang === 'PL' ? 'Anuluj' : 'Cancel'}
                            >
                              Anuluj
                            </button>
                            <button
                              ...
                              aria-label={connectionStatus === 'loading' ? (lang === 'PL' ? 'Weryfikacja...' : 'Verifying...') : (lang === 'PL' ? 'Zatwierdź i połącz' : 'Confirm and connect')}
                            >
          ...
          {!connectingId && (
            <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <button className="text-sm font-semibold text-primary-600 dark:text-primary-400 hover:underline" aria-label={t.cta.button}>
                {t.cta.button}
              </button>
            </div>
          )}
