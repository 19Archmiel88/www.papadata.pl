...
// components/Wizard.tsx - Zastąp całą funkcję startProvisioning tym kodem:
  ...
  const startProvisioning = async () => {
    ...
    const payload = {
      ...
      marketing_consent: notifications.marketing
    };

    try {
      // 2. Uruchamiamy animację paska postępu (wizualny efekt)
      const animationPromise = new Promise<void>((resolve) => {
        let val = 0;
        const timer = setInterval(() => {
          // Animuj tylko do 90%, ostatnie 10% czekamy na backend
          val = Math.min(90, val + Math.random() * 10 + 2);
          setProgress(val);
          if (val >= 90) {
            clearInterval(timer);
            resolve();
      ...
      // 3. WYSYŁAMY DANE DO CHMURY (równolegle z animacją)
      // To zapytanie uderzy w Twój Cloud Run!
      await Promise.all([
        registerTenant(payload), // Prawdziwy request
        animationPromise         // Sztuczna animacja
      ]);

      // 4. Sukces! Dokańczamy pasek i przekierowujemy
      setProgress(100);
      
      setTimeout(() => {
        persistIntegrationStates();
        try {
          // Tu zapisujemy token, który w przyszłości zwróci backend
          // Na razie, dla testów, zapisujemy placeholder, żeby Cię zalogowało
          window.localStorage.setItem("papadata_token", "demo-token-cloud-connected");
        } catch {}
        navigate("/dashboard");
      }, 1000);

    } catch (error: any) {
      // 5. Obsługa błędu (gdy backend nie odpowie lub zwróci błąd)
      console.error("Błąd rejestracji:", error);
      setShowTerminal(false);
      setSummaryError(
        lang === "PL"
          ? `Błąd rejestracji w chmurze: ${error.message || "Sprawdź konsolę."}`
          : `Cloud registration failed: ${error.message}`
      );
    }
  };
  const outerCard =
