...
import { ENTITLEMENTS_KEY, EntitlementFeature } from '../decorators/entitlements.decorator';
...
export class EntitlementsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const features = this.reflector.getAllAndOverride<EntitlementFeature[]>(ENTITLEMENTS_KEY, [
      context.getHandler(),
      context.getClass(),
    ]);
    ...
    const entitlements = await this.entitlementsService.getEntitlements(tenantId);

    const blocked = features.filter(
      (feature) => !this.entitlementsService.isFeatureAllowed(entitlements, feature)
    );
