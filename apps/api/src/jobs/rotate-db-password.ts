...
const main = async () => {
  ...
  const sslEnabled =
    getEnv("DATABASE_SSL_ENABLED", "true").toLowerCase() !== "false";
  const sslRejectUnauthorized =
    getEnv("DATABASE_SSL_REJECT_UNAUTHORIZED", "true").toLowerCase() !==
    "false";

  if (!projectId)
    throw new Error("Missing GOOGLE_CLOUD_PROJECT/GCP_PROJECT_ID");
