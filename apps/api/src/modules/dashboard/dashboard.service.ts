...
export class DashboardService {
  ...
  private buildCacheKey(name: string, params?: Record<string, string | undefined>): string {
    ...
    const serializedParams = params
      ? Object.entries(params)
          ...
          .join('&')
      : '';
    return ['dashboard', mode, name, serializedParams].filter(Boolean).join(':');
  }
  ...
  getOverview(params?: { timeRange?: string }): DashboardOverviewResponse {
    return this.withCache('overview', { timeRange: params?.timeRange }, () => {
      ...
      const timeMultiplier = timeRange === '1d' ? 0.032 : timeRange === '7d' ? 0.22 : 1;
      const baseSeed = timeRange === '1d' ? 19 : timeRange === '7d' ? 31 : 57;
      const pointsCount = timeRange === '1d' ? 12 : timeRange === '7d' ? 14 : 24;
      ...
      const revenueSpendSeries: TimeseriesPoint[] = labels.map((label, idx) => ({
        date: label,
        metrics: {
  ...
  getAds(params?: { timeRange?: string }): DashboardAdsResponse {
    return this.withCache('ads', { timeRange: params?.timeRange }, () => {
      const mode = getAppMode();
      const timeRange = resolveTimeRange(params?.timeRange);
      const timeMultiplier = timeRange === '1d' ? 0.05 : timeRange === '7d' ? 0.3 : 1;
      ...
      const spend = Array.from({ length: pointsCount }).map((_, idx) =>
        clamp(
          42_000 * timeMultiplier + seeded(idx, baseSeed) * (18_000 * timeMultiplier),
          1000,
          900_000
        )
      );
      const revenue = spend.map(
        (value, idx) => value * clamp(2.4 + seeded(idx + 5, baseSeed) * 2.2, 1.2, 6.5)
      );
      const roas = revenue.map((value, idx) => (spend[idx] ? value / spend[idx] : 0));
      const cpa = Array.from({ length: pointsCount }).map((_, idx) =>
        clamp(28 + seeded(idx + 9, baseSeed) * 26, 12, 95)
      );

      const spendRevenueSeries: TimeseriesPoint[] = labels.map((label, idx) => ({
        date: label,
      ...
      const campaigns: TableRow[] = Array.from({ length: 6 }).map((_, i) => {
        const baseSpend = (10_000 + seeded(i, baseSeed) * 35_000) * timeMultiplier;
  ...
  getProducts(params?: { timeRange?: string }): DashboardProductsResponse {
    return this.withCache('products', { timeRange: params?.timeRange }, () => {
      ...
      const timeMultiplier = timeRange === '1d' ? 0.2 : timeRange === '7d' ? 0.5 : 1;
      ...
      const skus: TableRow[] = Array.from({ length: 8 }).map((_, i) => {
        const revenue = (35_000 + seeded(i, baseSeed) * 90_000) * timeMultiplier;
        ...
        return {
          id: `SKU-${(i + 1).toString().padStart(3, '0')}`,
          dimensions: { name: `Product ${String.fromCharCode(65 + i)}`, stock },
          metrics: { revenue, profit, margin, returns, trend } as Partial<Record<KPIKey, number>>,
        };
      });
